...
async function parseLink(url) {
    console.log(`✨ Magic Parsing: ${url}`);

    // We reuse the browser logic (lightweight launch)
    const browser = await puppeteer.launch({
        args: chromium.args,
        defaultViewport: chromium.defaultViewport,
        executablePath: process.platform === 'win32'
            ? 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe' // Hardcoded for speed on user machine
    ...
    try {
        ...
        // Fast timeout, we just want meta tags
        ...
        // Use our helpers to parse extracted text
        // If it's a Facebook event, description usually contains the date/time
        const dateRaw = parseFacebookPost(meta.description).dateRaw || "";
        const timeRaw = extractTime(meta.description) || "19:00";
        const startTime = parseLithuanianDate(dateRaw, timeRaw);
        ...
        console.error("Link parse failed:", e);
...
module.exports = { runScout, parseLink, extractTime, parseLithuanianDate };
