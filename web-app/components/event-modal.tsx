...
import { X, Camera, Image as ImageIcon, Clock, MapPin, ExternalLink, Calendar, Tag } from 'lucide-react';
    ...
    return (
        <div className="w-full h-full relative bg-black group cursor-pointer" onClick={() => setShowControls(!showControls)}>
            ...
            <div className={`absolute top-0 left-0 right-0 p-6 flex justify-between items-center z-50 transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                ...
                <button onClick={(e) => { e.stopPropagation(); onClose(); }} className="pointer-events-auto p-2.5 rounded-full bg-black/20 backdrop-blur-xl text-white border border-white/10 hover:bg-white/20 transition-all active:scale-95 text-center flex items-center justify-center">
                    <X size={22} />
                </button>
            </div>

            {/* 3. Bottom Content */}
            <div className={`absolute bottom-0 left-0 right-0 z-40 p-6 pb-20 md:pb-8 text-white flex flex-col gap-5 max-h-[65%] overflow-y-auto scrollbar-none transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
